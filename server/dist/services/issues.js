    ...
    if (rawSearchInput) {
        ...
        for (const issueIdChunk of chunkList(issueIds, ISSUE_LIST_RELATED_QUERY_CHUNK_SIZE)) {
            const rows = await dbOrTx
                ...
                .from(issueComments)
                .where(and(eq(issueComments.companyId, companyId), inArray(issueComments.issueId, issueIdChunk), sql `${issueComments.body} ILIKE ${containsPattern} ESCAPE '\\'`));
            for (const row of rows)
