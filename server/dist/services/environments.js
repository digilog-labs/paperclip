...
import { ENVIRONMENT_DRIVERS, ENVIRONMENT_LEASE_CLEANUP_STATUSES, ENVIRONMENT_LEASE_POLICIES, ENVIRONMENT_LEASE_STATUSES, ENVIRONMENT_STATUSES, } from "@paperclipai/shared";
const DEFAULT_LOCAL_ENVIRONMENT_NAME = "Local";
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION = "Default execution environment for Paperclip runs on this machine.";
function cloneRecord(value, fallback = null) {
...
function toEnvironmentLease(row) {
    return {
        id: row.id,
        companyId: row.companyId,
        ...
        cleanupStatus: readEnum(row.cleanupStatus, ENVIRONMENT_LEASE_CLEANUP_STATUSES, "environment lease cleanup status"),
        metadata: cloneRecord(row.metadata),
        createdAt: row.createdAt,
        updatedAt: row.updatedAt,
    };
}
export function environmentService(db) {
    return {
        list: async (companyId, filters = {}) => {
            const conditions = [eq(environments.companyId, companyId)];
            if (filters.status)
                conditions.push(eq(environments.status, filters.status));
            if (filters.driver)
                conditions.push(eq(environments.driver, filters.driver));
            const rows = await db
                .select()
                .from(environments)
        ...
        ensureLocalEnvironment: async (companyId) => {
            const now = new Date();
            const row = await db
                .insert(environments)
                .values({
        ...
        update: async (id, patch) => {
            ...
            if (patch.driver !== undefined)
                values.driver = patch.driver;
            if (patch.status !== undefined)
                values.status = patch.status;
            if (patch.config !== undefined)
                values.config = patch.config;
            if (patch.metadata !== undefined)
                values.metadata = patch.metadata ?? null;
            const row = await db
                .update(environments)
                .set(values)
        ...
        listLeases: async (environmentId, filters = {}) => {
            const conditions = [eq(environmentLeases.environmentId, environmentId)];
            if (filters.status)
                conditions.push(eq(environmentLeases.status, filters.status));
            const rows = await db
                .select()
                .from(environmentLeases)
                .where(and(...conditions))
        ...
        releaseLease: async (id, status = "released", options) => {
            const now = new Date();
            const row = await db
        ...
        updateLeaseMetadata: async (id, metadata) => {
            const row = await db
                .update(environmentLeases)
                .set({
                metadata,
        ...
        releaseLeasesForRun: async (heartbeatRunId, status = "released") => {
            const now = new Date();
            const rows = await db
                .update(environmentLeases)
                .set({
