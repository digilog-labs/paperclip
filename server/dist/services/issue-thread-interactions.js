...
function shouldReturnAcceptedConfirmationToCreatorAgent(args) {
    if (args.current.kind !== "request_confirmation")
        return false;
...
async function expireStaleRequestConfirmationTarget(db, args) {
    if (args.row.kind !== "request_confirmation" || args.row.status !== "pending")
        return null;
    ...
    return {
        ...
        expireRequestConfirmationsSupersededByComment: async (issue, comment, actor) => {
            ...
            const rows = await db
                .select()
                .from(issueThreadInteractions)
                .where(and(eq(issueThreadInteractions.companyId, issue.companyId), eq(issueThreadInteractions.issueId, issue.id), eq(issueThreadInteractions.kind, "request_confirmation"), eq(issueThreadInteractions.status, "pending")));
