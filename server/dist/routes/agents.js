...
export function agentRoutes(db, options = {}) {
    ...
    router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
        ...
        if (agentEnv) {
            await secretsSvc.syncEnvBindingsForTarget?.(companyId, { targetType: "agent", targetId: agent.id }, agentEnv);
        }
        const actor = getActorInfo(req);
        await logActivity(db, {
            companyId,
    ...
    router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
        ...
        if (touchesAdapterConfiguration) {
            const agentEnv = asRecord(agent.adapterConfig)?.env;
            await secretsSvc.syncEnvBindingsForTarget?.(agent.companyId, { targetType: "agent", targetId: agent.id }, agentEnv);
        }
        await logActivity(db, {
            companyId: agent.companyId,
            actorType: actor.actorType,
