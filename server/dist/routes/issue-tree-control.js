...
export function issueTreeControlRoutes(db) {
    ...
    router.get("/issues/:id/tree-holds/:holdId", async (req, res) => {
        ...
        assertCompanyAccess(req, root.companyId);
        const hold = await treeControlSvc.getHold(root.companyId, req.params.holdId);
    ...
    router.post("/issues/:id/tree-holds/:holdId/release", validate(releaseIssueTreeHoldSchema), async (req, res) => {
        ...
        assertCompanyAccess(req, root.companyId);
        const actor = getActorInfo(req);
        const hold = await treeControlSvc.releaseHold(root.companyId, root.id, req.params.holdId, {
            ...req.body,
            actor: {
                actorType: actor.actorType,
                actorId: actor.actorId,
