...
import { accessService, agentService, issueService, logActivity, projectService, } from "../services/index.js";
...
export function environmentRoutes(db, options = {}) {
    ...
    async function assertCanMutateEnvironments(req, companyId) {
    ...
    async function actorCanReadEnvironmentConfigurations(req, companyId) {
        assertCompanyAccess(req, companyId);
        if (req.actor.type === "board") {
            if (req.actor.source === "local_implicit" || req.actor.isInstanceAdmin)
                return true;
    ...
    router.patch("/environments/:id", validate(updateEnvironmentSchema), async (req, res) => {
        ...
        if (patch.config !== undefined || patch.driver !== undefined) {
            await secrets.syncSecretRefsForTarget(environment.companyId, { targetType: "environment", targetId: environment.id }, await collectEnvironmentSecretRefs({ db, environment }));
        }
        await logActivity(db, {
