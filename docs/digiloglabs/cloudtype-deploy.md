...
Cloudtype 기본 인스턴스(~1GB RAM)에서 UI `vite build`가 OOM 날 수 있습니다.

**대응 (코드):** `CLOUDTYPE_BUILD=1` 시 Vite 청크 분할 + `build:deploy`(tsc 생략). `NODE_OPTIONS=--max-old-space-size=1536`.

**권장:**

1. Cloudtype **빌드** 단계에서 `pnpm run build` 실행 (시작 시 빌드 X) — 빌드 VM이 더 크면 성공하기 쉬움  
2. 빌드/런타임 env: `NODE_OPTIONS=--max-old-space-size=2048`, `CLOUDTYPE_BUILD=1`  
3. 그래도 실패 시 플랜 **메모리 2GB+** 또는 **Dockerfile** 템플릿

---

## `Cannot find module server/dist/index.js`

Node 템플릿 런타임 이미지에 **빌드 결과가 포함되지 않을** 수 있습니다. `master`의 `npm start`는 `scripts/cloudtype-start.mjs`가 없으면 install + `build:cloudtype` 후 서버를 띄웁니다.

Cloudtype **빌드** 명령도 `pnpm run build` 로 두면 첫 기동이 빨라집니다.
